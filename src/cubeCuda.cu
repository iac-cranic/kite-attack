...
//#elif defined GRAIN128_CIPHER 
    ...
    for(i = 0 ; i < count_shared ; i++){
        ...
        a = k_mickey2(key0, key1, key2, iv_curr0, iv_curr1, iv_curr2, num_round, MICKEY_IV_LEN);
        sumA = sumA ^ a;
        a = k_mickey2(key3, key4, key5, iv_curr0, iv_curr1, iv_curr2, num_round, MICKEY_IV_LEN);
        sumB = sumB ^ a;
        a = k_mickey2(key6, key7, key8, iv_curr0, iv_curr1, iv_curr2, num_round, MICKEY_IV_LEN);
        sumC = sumC ^ a;
#else
        //ERROR: you must specify a supported cipher
#endif
...
#elif defined MICKEY2_CIPHER
	setBitMickey2(&(key_vett[index]),i,1);
#endif
...
#elif defined MICKEY2_CIPHER
            out = ( (host_k2_output[index_out] >> (31- cob) ) & 0x1) ; //TODO
#endif
            if(out != 0) {
                if(0 == k) {
                    fprintf(fout, "%u +", 1 );
